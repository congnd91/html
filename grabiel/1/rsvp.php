...
<form id="rsvp-now" action="" class="tribe-tickets-rsvp cart <?php echo esc_attr($messages_class); ?>" method="post" enctype='multipart/form-data'>
    <h6 class="tribe-events-tickets-title tribe--rsvp">
        <?php echo esc_html('Online Anmeldung'); ?>
    </h6>
            ...
            <tr>
                ...
                <td class="tribe-ticket quantity" data-product-id="<?php echo esc_attr($ticket_id); ?>">
                    ...
                    <?php if ($is_there_any_product_to_sell) : ?>
                        <input type="number" class="tribe-tickets-quantity" step="1" min="0" <?php if (-1 !== $available) : ?> max="<?php echo esc_attr($available); ?>" <?php endif; ?> name="quantity_<?php echo absint($ticket_id); ?>" value="0" <?php disabled($must_login); ?>>
                        <?php if (-1 !== $available && (0 === $threshold || $available <= $threshold)) : ?>
                            <span class="tribe-tickets-remaining">
                                <span class="available-stock" data-product-id="<?php echo esc_attr($ticket_id); ?>">
                                    <?php echo sprintf(esc_html__('Anzahl Teilnehmer', 'event-tickets'), esc_html($readable_amount)); ?>
                                </span>
                            </span>
                    ...
                    <?php elseif (!$ticket->is_in_stock()) : ?>
                        <span class="tickets_nostock"><?php esc_html_e('Ausgebucht', 'event-tickets'); ?></span>
                    <?php endif; ?>
                </td>
                <td class="tickets_name" colspan="2">
                    <?php echo esc_html(($ticket->show_description() ? $ticket->description : '')); ?>
                    <?php echo sprintf(esc_html__('aktuell noch %1$s Pläze verfügbar', 'event-tickets'), esc_html($readable_amount)); ?>
                </td>
            </tr>
        <?php
            ...
            <tr class="tribe-tickets-meta-row">
                <td colspan="4" class="tribe-tickets-attendees">
                    ...
                    <table class="tribe-tickets-table">
                        ...
                        <tr class="tribe-tickets-invoice-same-as-attendee-one-row">
                            <td>
                                <label for="tribe-tickets-invoice-same-as-attendee-one"><?php esc_html_e('Rechnungsadresse entspricht Angaben Teilnehmer 1', 'event-tickets'); ?>:</label>
                            </td>
                            <td colspan="3">
                                <select name="attendee[invoice_check]" class="db-invoice-check">
                                    <option value="yes">Ja</option>
                                    <option value="no">Nein</option>
                                </select>
                            </td>
                        </tr>
                        <tr class="tribe-tickets-firma">
                            <td>
                                <label for="tribe-tickets-firma"><?php esc_html_e('Firma (Optional)', 'event-tickets'); ?>:</label>
                            </td>
                            <td colspan="3">
                                <input type="text" name="attendee[firma]" id="tribe-tickets-firma" value="<?php echo esc_html($firma); ?>">
                            </td>
                        </tr>
                        <tr class="tribe-tickets-email-row">
                            ...
                            <td colspan="3">
                                <input type="email" name="attendee[email]" id="tribe-tickets-email" value="<?php echo esc_html($email); ?>" required>
                            </td>
                        </tr>

                        <tr class="tribe-tickets-order_status-row">
                            <td>
                                <label for="tribe-tickets-order_status"><?php esc_html_e('Status', 'event-tickets'); ?>:</label>
                            </td>
                            <td colspan="3">
                        ...
                        <!--option no content-->
                        <tr class="tribe-tickets-vorname-row db-no" style="display:none">
                            <td>
                        ...
                        <tr class="tribe-tickets-full-name-row db-no" style="display:none">
                            <td>
                        ...
                        <tr class="tribe-tickets-plz-row db-no" style="display:none">
                            <td>
                                <label for="tribe-tickets-plz"><?php esc_html_e('PLZ / Ort', 'event-tickets'); ?>:</label>
                            </td>
                            <td colspan="3">
                                <input type="text" name="attendee[plz]" id="tribe-tickets-plz" value="">
                            </td>
                        </tr>

                        <tr class="tribe-tickets-telefon-row db-no" style="display:none">
                            <td>
                                <label for="tribe-tickets-telefon"><?php esc_html_e('Telefon', 'event-tickets'); ?>:</label>
                            </td>
                            <td colspan="3">
                                <input type="text" name="attendee[telefon]" id="tribe-tickets-telefon" value="">
                            </td>
                        </tr>
                        ...
                        <tr class="tribe-tickets-attendees-agree">
                            <td colspan="4">
                                <input type="checkbox">
                                <label for="tribe-tickets-attendees-agree">
                                    <?php esc_html_e('JA, ich habe die agbs galesen und akzeptiere diese', 'event-tickets'); ?>
                                </label>
                            </td>
                        </tr>
            ...
            <tr>
                <td colspan="4" class="add-to-cart">
                    ...
                    <?php else : ?>
                        ...
                        <button type="submit" name="tickets_process" value="1" class="tribe-button tribe-events-button tribe-button--rsvp">
                            <?php echo esc_html(sprintf(_x('Anmelden', 'event-tickets'), tribe_get_rsvp_label_singular('tickets_process_button_text'))); ?>
                            <?php // Original String - echo esc_html( sprintf( _x( 'Confirm %s', 'tickets process button text', 'event-tickets' ), tribe_get_rsvp_label_singular( 'tickets_process_button_text' ) ) );
                            ?>
                        </button>
                    <?php endif; ?>
                </td>
